...
public class ClassApp {
	...
	public static void main(String[] args) {
		...
		List<List<OnlineClass>> mermerClasses = new ArrayList<>();
		mermerClasses.add(springClasses);
		mermerClasses.add(javaClasses);
		...
		// TODO
		springClasses.stream().filter(Predicate.not(OnlineClass::isClosed)).forEach(c -> System.out.println(c.getId()));

		System.out.println("수업 이름만 모아서 스트림 만들기");
		// TODO
		springClasses.stream().map(OnlineClass::getTitle).forEach(t -> System.out.println(t));

		mermerClasses.stream().flatMap(Collection::stream).forEach(list -> System.out.println(list.getId()));
